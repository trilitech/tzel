...
fn cmd_rollup_sync(
    ...
    // run's `seen_cms`. (`load_pool_balances` re-resolves head; the
    // `_at_block` overload is the consistent variant.)
    let pool_balances =
...
mod network_profile_tests {
    ...
    /// index 0 is found in run 1; the recoverable note at index 75
    /// (after DEFAULT_CHECKPOINT_EVERY=50) is found in run 2.
    #[test]
    #[serial_test::serial(sync_checkpoint_hook)]
    fn cmd_rollup_sync_resumes_from_checkpointed_cursor() {
        ...
        let tree_size: u64 = (DEFAULT_CHECKPOINT_EVERY as u64) * 3 + 5;
        let block_hash = "BLresumecheckpoint";

        let mut recoverable: std::collections::HashMap<u64, (u64, F)> =
            std::collections::HashMap::new();
        recoverable.insert(7, (101, felt_tag(b"resume-note-7")));
        recoverable.insert(75, (202, felt_tag(b"resume-note-75")));
        recoverable.insert(140, (303, felt_tag(b"resume-note-140")));
        ...
        let trigger_at = (DEFAULT_CHECKPOINT_EVERY as usize) * 2;
        ...
        cmd_rollup_sync(wallet_path_str, &profile, DEFAULT_CHECKPOINT_EVERY).expect("first sync should yield");
        ...
        assert!(
            recovered_run1.contains(&75),
            "note at index 75 must be recovered before yield"
        );
        assert!(
            !recovered_run1.contains(&140),
            "note at index 140 lies past the yield boundary and must NOT be recovered yet"
        );
        ...
        cmd_rollup_sync(wallet_path_str, &profile, DEFAULT_CHECKPOINT_EVERY).expect("second sync should finish");
        ...
        assert!(recovered_final.contains(&75));
        assert!(
            recovered_final.contains(&140),
    ...
    fn cmd_rollup_sync_recovers_from_stale_yield_sentinel() {
        ...
        // Pre-create the sentinel with a deliberately-dead PID. We
        // use 999999 — well above the typical pid_max ceiling on
        // Linux (32_768 by default, 4M with high-pid kernels). The
        // CLI's stale check probes /proc/999999; on any ubuntu / dev
        // environment that path doesn't exist.
        let yield_path = wallet_yield_path(wallet_path_str);
        std::fs::write(&yield_path, "999999\n").expect("write stale sentinel");
