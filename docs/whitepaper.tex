...
separation). The L1 deposit transaction credits the public rollup balance
key \texttt{deposit:<hex(intent)>} for exactly \(v_{pub} + fee + producer\_fee\)
mutez, no more, no less. There is no separate \(deposit\_secret\): the L1
deposit's own L1 signature is the spend authorization, and the
deposit-id binding propagates to every detail of the shield.
...
recomputed \(intent\). The rollup additionally recomputes \(intent\) from the
request fields and rejects any mismatch, then verifies note-data hashes and
the required burned fee. The deposit balance MUST equal exactly the consumed
debit \(v_{pub} + fee + producer\_fee\); on success the kernel drains it to
zero in a single shot. There is no partial drain and no top-up.
