...
same \texttt{deposit:<hex(pubkey\_hash)>} recipient string aggregate into
one balance. The \texttt{deposit:<hex(pubkey\_hash)>} string is public on
...
\]
The kernel reads \(balance = \mathrm{pool}[pubkey\_hash]\), rejects unless
\(balance \ge v_{pub} + fee + producer\_fee\), debits the pool, and
appends both notes. Pool overfunding is fine — the surplus stays
available for future shields. The user picks
\((v_{pub}, fee, producer\_fee)\) at shield time, so fee revisions
between deposit and shield are a wallet-side retry, not a stranded slot.
...
that the note has positive value. There is no privileged rollup
operator: the producer fee is paid to whichever DAL slot publisher
chooses to include the transaction, in a permissionless market where
publishers pay bakers for slot inclusion and keep the spread between
producer fees collected and L1 inclusion costs. Publishers therefore
gate inclusion off-chain on \(producer\_otag\) matching their own
address — a wallet routing fees elsewhere simply won't be bundled. The
kernel does not — and intentionally need not — cross-check the producer
receiver against any on-chain value, because there is no canonical
receiver to check against.

\paragraph{Top-ups and partial drains.} A user can send multiple L1
tickets to the same \texttt{deposit:<hex(pubkey\_hash)>} recipient; each
