...
\begin{itemize}
  ...
  \item a set of public bridge deposit receipts keyed by intent-bound deposit
        identifiers (each id is a hash of the entire shield it authorizes:
        recipient note, producer-fee note, fees, and authorization domain),
        used only to move value into the shielded pool;
...
\midrule
Shield & a bridge deposit receipt keyed by an intent-bound deposit identifier & a user note and a DAL-producer fee note \\
...
transparent account balance. It is a bridge deposit receipt keyed by an
\emph{intent-bound} deposit identifier:

\[
intent = \Hash_{intent}(auth\_domain,\, v_{pub},\, fee,\, producer\_fee,\,
                        \cm_{user},\, \cm_{producer},\,
                        mh_{user},\, mh_{producer}),
\quad deposit\_id = intent.
\]

\(\Hash_{intent}\) is the sequential left-fold using the \texttt{sighSP\_\_}
personalization with leading type tag \(0x03\) (the same primitive as the
spend-authorization sighash but with a distinct type tag for domain
separation). The L1 deposit transaction is addressed to the canonical
recipient string \texttt{deposit:<hex(intent)>}, and the kernel allocates a
fresh per-deposit \emph{slot} for that L1 ticket. The slot's \emph{key}
is a \(slot\_id\) drawn from a kernel-controlled monotonic counter
(depositors cannot influence, predict, or collide slot ids); the slot's
\emph{content} is \((intent, amount)\). There is no separate
\(deposit\_secret\): the L1 deposit's own L1 signature is the spend
authorization, and the deposit-id binding propagates to every detail of the
shield.

\paragraph{Per-deposit slots and dust resistance.} Two different L1
tickets addressed to the same \texttt{deposit:<hex(intent)>} recipient
string each allocate their own slot rather than accumulating into a
shared balance, and even tickets with identical \((intent, amount)\)
content still get distinct slot ids. This matters because the
\texttt{deposit:<hex(intent)>} string is public on L1: an attacker could
observe a victim's deposit, send a 1-mutez ticket to the same recipient
string, and (under a single shared balance scheme) push the balance
above the bound debit, permanently bricking the legitimate shield
(which requires the balance to equal the debit exactly). With
per-deposit slots, the dust ticket creates an orphan slot and the
legitimate slot's content is unchanged.

The public outputs of shield are

\[
[auth\_domain,\, v_{pub},\, fee,\, producer\_fee,\, \cm_{user},\, \cm_{producer},\, deposit\_id,\,
 mh_{user},\, mh_{producer}].
\]

The circuit proves the user commitment, the producer-fee commitment, the
condition \(producer\_fee > 0\), and that \(deposit\_id\) equals the
recomputed \(intent\). The shield request additionally carries a
\(deposit\_slot\) field selecting which kernel-side slot to drain. The
rollup recomputes \(intent\) from the request fields and rejects any
mismatch, verifies note-data hashes, and looks up the slot. It accepts only
if the slot's \(intent\) equals \(deposit\_id\) and its \(amount\) equals
\(v_{pub} + fee + producer\_fee\) exactly. On success the slot is
tombstoned in a single shot. There is no partial drain and no top-up; an
over- or under-deposit leaves an orphan slot but cannot affect any other.
...
grants spend authority because the intent commits to all of them: any
modification by an untrusted prover yields a different \(deposit\_id\) whose
L1 balance is empty. This makes shield safe to delegate to an external proving
service without any in-circuit signature.

\paragraph{auth\_domain freeze.} Because \(intent\) commits to
\(auth\_domain\), any reconfiguration of the deployment's
\(auth\_domain\) between the wallet computing \(intent\) and the L1
ticket landing would silently strand the deposit. The kernel therefore
freezes \(auth\_domain\) on first verifier configuration: subsequent
\texttt{configure\_verifier} messages may rotate program hashes or the
operator's producer-fee receiver, but any attempt to change
\(auth\_domain\) is rejected, even on a still-pristine ledger. Other
verifier-config fields remain rotatable while pristine.

\paragraph{Producer-fee receiver binding.} The producer-fee note
recipient is a private input; the circuit only proves that
\(\cm_{producer}\) commits to some
\(producer\_otag = \Hash_{owner}(auth\_root, pub\_seed, nk\_tag)\), not
that \(producer\_otag\) equals the operator's address. To prevent a
misconfigured wallet profile from silently routing fees to a
non-operator receiver, the kernel's verifier config carries an
\(operator\_producer\_owner\_tag\) field, signed alongside
\(auth\_domain\) and the program hashes. Reference wallets refuse to
submit shield, transfer, or unshield unless the \(owner\_tag\) derived
from the profile-supplied DAL-fee receiver equals the published
\(operator\_producer\_owner\_tag\). This is reference behavior, not a
consensus rule — a custom wallet that skips it merely loses its own fee
notes.

\paragraph{Orphan-slot drain.} If an attacker mirror-deposits a victim's
\texttt{deposit:<hex(intent)>} recipient string with the same
\(amount\) as the victim's settled shield, the extra L1 ticket allocates
an orphan slot that holds real mutez but is not consumed by the victim's
shield. Because the victim's wallet still holds the full deposit
witness, it can re-shield the original recipient \(\cm\) against the
orphan slot, recovering the L1 mutez. Reference wallets expose this
through the same shield command with an explicit slot override (so
"orphan drain" is not a separate operation, just a non-canonical slot
target requiring the user to acknowledge the privacy cost). The drained
slot produces a duplicate recipient commitment at a new tree position;
the two leaves are publicly correlatable to the same intent but each
gets a distinct nullifier (nullifiers fold in tree position) and is
independently spendable. Mirror-depositing is therefore an asymmetric
attack: the attacker burns mutez to a slot only the victim's witness
can drain, and the only "damage" is the on-chain intent linkage exposed
when the victim chooses to recover the funds.
...
\(fee\) is burned. There is no separate direct-withdraw path for the deposit
receipt; only Shield can consume it. Under the intent-bound scheme,
\(deposit\_id\) is a hash of off-chain witness data that the wallet persists
and that is not revealed by the L1 deposit alone, so a third party who merely
observes the deposit cannot construct an alternative shield against the same
\(deposit\_id\).
...
\begin{itemize}
  ...
  \item equality between posted note data and the note-data hashes in the
        proof outputs;
  \item correct binding of shield to the exact intent-bound deposit receipt
        being consumed (the kernel recomputes \(intent\) from the request
        fields and requires \(deposit\_id == intent\); the L1 balance must
        also equal exactly \(v_{pub} + fee + producer\_fee\));
...
\begin{itemize}
  ...
  \item \textbf{Shield is intent-bound and safe to delegate.} Every
        prover-rewritable shield field --- \(auth\_domain\), value, fees,
        recipient and producer commitments, ciphertext memo hashes ---
        is folded into \(deposit\_id\). Cross-deployment replay fails
        because \(auth\_domain\) is in the intent, and prover-side
        redirection fails because the kernel recomputes the intent from
        request fields. The cost is that shield is single-shot and
        exact-amount: an L1 deposit that does not equal the bound debit
        is unrecoverable, since no other intent collides with the same
        \(deposit\_id\). Wallet UX must therefore size deposits precisely.
\end{itemize}
