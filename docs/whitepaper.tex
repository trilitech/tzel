...
\(blind\) is a per-deposit randomness the wallet derives deterministically
from \(master\_sk\), so distinct deposits to the same auth tree get
distinct pool ids by default and the wallet can recover all its pools
from the seed alone.
...
\paragraph{auth\_domain freeze.} Because \(pubkey\_hash\) commits to
\(auth\_domain\), any reconfiguration of the deployment's
\(auth\_domain\) between the wallet computing \(pubkey\_hash\) and the L1
