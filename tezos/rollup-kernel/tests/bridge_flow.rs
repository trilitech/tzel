...
// Phase C blocked: this test uses a checked-in STARK proof generated for
// the pre-Phase-C 3-output transfer / 1-change unshield Cairo layouts.
// The new circuits emit 4-output transfers and 2-change unshields, so the
// public-output length check rejects these old proofs (10 vs 12 fields).
// Re-enable once the bridge fixture is regenerated by `reprove` against
// the post-Phase-C Cairo executables. Tracked alongside the wallet
