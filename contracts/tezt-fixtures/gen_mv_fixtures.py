...
program_hash, auth_domain = feltbe(shield[2]), feltbe(shield[4])  # real op: [3]=output_count, [4]=auth_domain
...
LEAF_TO_MV = [327071585,1981680103,733719417,886400460,805003444,384535660,1686938097,1077085843]  # corrected: live leaf_to_mv preprocessed root (was stale [1329128718...])
...
leaves = f'{{ {feltlist(shield)} ; {feltlist(shield)} ; {feltlist(shield)} ; {feltlist(shield)} }}'  # mv_parallel e2e proves 4 shield leaves
...
v_note, fee, producer_fee = int(shield[6], 16), int(shield[7], 16), int(shield[8], 16)  # real op +1
